...
void koch_helper(const glm::vec2& p0, const glm::vec2& p1, const glm::vec2& normal, std::vector<float>& vec, std::vector<uint32_t>& ind, uint32_t depth) {
    if (depth <= 1) {
        return;
    }
    ...
    koch_helper(p0, q0, normal, vec, ind, depth - 1);
    auto index_q0 = vec.size() / glm::vec2::length();
    vec.push_back(q0.x);
    vec.push_back(q0.y);

    koch_helper(q0, a, cc_normal(a - q0), vec, ind, depth - 1);
    auto index_a = vec.size() / glm::vec2::length();
    vec.push_back(a.x);
    vec.push_back(a.y);

    koch_helper(a, q1, cc_normal(q1 - a), vec, ind, depth - 1);
    auto index_q1 = vec.size() / glm::vec2::length();
    vec.push_back(q1.x);
    vec.push_back(q1.y);

    koch_helper(q1, p1, normal, vec, ind, depth - 1);
...
void create_snowflake(const std::shared_ptr<Engine::Vertex_buffer>& vertex_buffer,const std::shared_ptr<Engine::Index_buffer>& index_buffer,uint32_t depth) {
    ...
    koch_helper(a, b, cc_normal(b - a), vec,triangle_indices, depth);
    auto index_b = vec.size()/glm::vec2::length();
    vec.push_back(b.x);
    vec.push_back(b.y);

    koch_helper(b, c, cc_normal(c - b), vec,triangle_indices, depth);
    auto index_c = vec.size()/glm::vec2::length();
    vec.push_back(c.x);
    vec.push_back(c.y);

    koch_helper(c, a, cc_normal(a - c), vec,triangle_indices, depth);
    vec.push_back(a.x);
    vec.push_back(a.y);
